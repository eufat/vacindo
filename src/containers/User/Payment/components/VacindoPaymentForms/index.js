...
import VacindoMessage from '../../../../../components/VacindoMessage';
...
class VacindotransferForms extends Component {
  state = {
  ...
  voucherIsValid = () => new Promise(async (resolve, reject) => {
    const now = new Date();
    const { dispatch } = this.props;
    let voucherForm = this.state.form.voucher;
    voucherForm = { ...voucherForm, code: toUpperCase(voucherForm.code) };
  ...
  render() {
    ...
    const transfer = () => (
      <div>
        <VacindoMessage>
          BNI 0459422250 a.n. Tamam Abdu K
        </VacindoMessage>
        <TextField
...
VacindotransferForms.propTypes = {
...
export default withStyles(styleSheet)(connect(mapStateToProps)(VacindotransferForms));
