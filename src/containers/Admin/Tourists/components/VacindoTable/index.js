...
import { retrieveTourists, deleteTourist } from '../../../actions';
...
class VacindoTable extends React.PureComponent {
  ...
  setRows = (tourists, rows) => {
    const totalCount = this.props.touristsCount;
    this.setState({
      ...this.state,
      loading: false,
      rows,
  ...
  fetchData() {
    ...
    retrieveTourists(
      ...
      const rows = map(tourists, (value, uid) => ({ ...value, uid }));
      this.setRows(tourists, rows);
    });
  }
